...
export const getAll = async () => await OrderModel.find().populate("books").populate("user", "email");
export const getOne = async (id) => await OrderModel.findById(id).populate("books").populate("user", "email");
export const getCustomerOrder = async (userId) => {
...
export const update = async (orderId, payload, vendorId) => {
    try {
        ...
        if (!book)
            throw new Error("Book does not belong to this vendor");
...
export const post = async (payload) => await OrderModel.create(payload);
export const deleteOne = async (id) => await OrderModel.findByIdAndDelete(id);
